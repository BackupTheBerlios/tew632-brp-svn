...
disable_wmac_led()
{
    ...
    ar7100_gpio_out_val(7, 1); 
    ar7100_gpio_out_val(8, 1);  
}
...
#ifdef CONFIG_AR9100
    /* The gpio init and off is move to start_kernel to it */
    //ar7100_gpio_config_output(6); 
   	//ar7100_gpio_config_output(7);
    //ar7100_gpio_config_output(8);
    //ar7100_gpio_out_val(6, 1); 
    //ar7100_gpio_out_val(7, 1);
    //ar7100_gpio_out_val(8, 1);
	/* if old chipset need mask below 2 line */
    ar7100_reg_wr(AR9100_OBS_GPIO_1, 0x16);//obs1 set to gpio 6 mux
    ar7100_reg_wr(AR9100_OBS_OE, 0x40);//obs to gpio 6
#endif

}

void wireless_switchalbe_led_init(int gpio_pin)
{
    printk("wireless_switchalbe_led_init: gpio_pin =%d \n", gpio_pin);
    ar7100_gpio_out_val(gpio_pin, 0);//amber on 

	/* if old chipset need mask below 2 line */
    ar7100_reg_wr(AR9100_OBS_GPIO_1, 0x16<<(5*(gpio_pin-6)));//obs1 set to gpiox mux
//    ar7100_reg_wr(AR9100_OBS_OE, 0x40);
    ar7100_reg_wr(AR9100_OBS_OE, 1<< gpio_pin);//obs to gpiox		
}
EXPORT_SYMBOL(wireless_switchalbe_led_init);
/*
 * -------------------------------------------------
 * Early printk hack
 */
