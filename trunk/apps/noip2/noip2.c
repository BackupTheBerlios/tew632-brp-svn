...
#define OPTCHARS				"CYU:Fc:hSMi:K:I:u:p:R:T:wk:a:n:"
#define ARGC						1
#define ARGF						2
#define ARGY						4
#define ARGU						8
...
int main(int argc, char *argv[]){
	...
	{
retry:
		resule = dynamic_update(port_to_use, ddns_type, curr_wan_ip, send_buf);
		time(&clock_t);
		tm = localtime(&clock_t) ;
		strftime(buf,sizeof(buf),"%A, %B %d, %Y %X",tm);

		if ((resule != SUCCESS) && (resule != CYBERGATEDDNS)) 
		{
			if(resule == BADAUTH)
			{
			...
			DEBUG_MSG("In noip2, dynamic_update() fail !!!\n");
			count++;
			DEBUG_MSG("try %d times for 5 sec\n", count);	
			if(count > 4)
			{
/* Chun modify:
...
/* Chun end */
		...
		{
			syslog(LOG_INFO, "NOIP2: update success\n");
			//_system("killall -SIGUSR1 wantimer"); WHY???????
			/* Tell ntptime the update is successfull and 
			 * ip and hostname need to be saved*/
#if 0//for cdrouter DynDns client doesn't rely upon DNS to determine if an update is necessary
				_system("killall -SIGUSR1 noiptime");
#endif				
			save_to_file();
			count = 0;
			break;
	...
	if ( (curr_wan_ip) && (curr_wan_ip != old_ddns_ip) ) {							 
		if ( get_our_visible_IPaddr(config_Rhostname, wan_IP) != SUCCESS ) {
			DEBUG_MSG("Not connected to Net!\n");
			continue;				
		}
...
void save_to_file(void)
{
	...
	init_file(DDNS_STATUS_FILE);
	save2file(DDNS_STATUS_FILE,"SUCCESS|%s|%s|%s|%s",config_Rhostname,get_ipaddr(temp),ddns_type==DYNDNS_TYPE? "DynDns.org" : "dlinkddns.com", buf);
}
...
int dynamic_update(int port, int d_type, unsigned long my_ip, char *send_buf){
	int	fd, web_converse_ret, retval;
	...
	switch (d_type) {
		...
		case NOIP_TYPE:
			printf("NOIP_TYPE");
			strcpy(send_buf, "GET /dns?");
			sprintf(tmp_buf, "username=%s", config_username);
			strcat(send_buf, tmp_buf);
			sprintf(tmp_buf, "&password=%s", config_password); 
			strcat(send_buf, tmp_buf);

			sprintf(tmp_buf, "&hostname=%s", config_Rhostname); 
			strcat(send_buf, tmp_buf);
...
*/
	...
	if ((web_converse_ret !=SUCCESS) && (web_converse_ret != CYBERGATEDDNS)){	
		DEBUG_MSG("converse_with_web_server() fail !!!\n"); 
		return web_converse_ret;
	}
	else if(web_converse_ret == CYBERGATEDDNS)
	{
		DEBUG_MSG("Ready to send sendary packet to server !!!\n");
		return CYBERGATEDDNS;
	}
	else
		DEBUG_MSG("Dynamic update IP Successful !!!!\n");
