	...
	list_t **msg, char **errstr)
{
    ...
    *lmtp_error_msgs = NULL;
    ...
    /* connect */
    if ((e = smtp_connect(&srv, acc->host, acc->port, acc->timeout,
		    NULL, /*NULL*/&server_address, errstr)) != NET_EOK)
    {
		DEBUG_MSG("msmtp_sendmail failed error code =%d\n", e);
		e = exitcode_net(e);
		goto error_exit;
    }

	//jimmy modified
    //save2file(LOG_FILE_HTTP,"%s %s info smtp:SMTP (Email) server %s is at IP address %s:%d\n", nowTime(), LocalHostName, acc->host, server_address, acc->port);
	syslog(LOG_ERR,"%s %s info smtp:SMTP (Email) server %s is at IP address %s:%d\n", nowTime(), LocalHostName, acc->host, server_address, acc->port);
    ...
    /* initialize session */
    DEBUG_MSG("msmtp_sendmail: smtp_init\n");
    if ((e = smtp_init(&srv, acc->domain, msg, errstr)) != SMTP_EOK)
    {
		msmtp_endsession(&srv, 0);
		e = exitcode_smtp(e);
	...
	//	    msmtp_endsession(&srv, 1);
	//	    e = EX_UNAVAILABLE;
			DEBUG_MSG("msmtp_sendmail: the server does not support authentication\n");
		    goto send_without_auth;
		}
		
		DEBUG_MSG("msmtp_sendmail: smtp_auth: start\n");
		if ((e = smtp_auth(&srv, acc->host, acc->username, acc->password, 
				acc->ntlmdomain, acc->auth_mech, 
				msmtp_password_callback, msg, errstr)) 
			!= SMTP_EOK)
		{
		    msmtp_endsession(&srv, 0);
		    e = exitcode_smtp(e);
		    DEBUG_MSG("msmtp_sendmail: smtp_auth: error\n");
		    goto error_exit;
		}
		DEBUG_MSG("msmtp_sendmail: smtp_auth: end\n");
    }
send_without_auth:
    ...
    *mailsize = 0;
    if (read_recipients)
    {
		DEBUG_MSG("reading recipients from the command line and the mail\n");		
    ...
    {	
    	DEBUG_MSG("Don't need to read recipients from the command line and the mail\n");
		if ((e = smtp_send_mail(&srv, f, acc->keepbcc, mailsize, errstr)) 
		     	!= SMTP_EOK)
		{
		    printf("msmtp_sendmail: smtp_send_mail error\n");
		    msmtp_endsession(&srv, 0);
		    e = exitcode_smtp(e);
		    goto error_exit;
		}
    ...
    if (e != SMTP_EOK)
    {
		printf("msmtp_sendmail: smtp_end_mail_lmtp fail\n");
		msmtp_endsession(&srv, 0);
		e = exitcode_smtp(e);
		goto error_exit;
    }
	//jimmy modified
    //save2file(LOG_FILE_HTTP,"%s %s info smtp:SMTP client success sending email\n", nowTime(), LocalHostName);
	syslog(LOG_ERR,"%s %s info smtp:SMTP client success sending email\n", nowTime(), LocalHostName);
	printf("msmtp_sendmail OK \n");
	/* Ken modify for log full check */
	if( !stat(LOG_FILE_FULL, &file_buf) ){		
		_system("rm -rf %s",LOG_FILE_FULL);		
	}	
...
void msmtp_print_version(void)
{
    ...
    DEBUG_MSG(_("%s version %s\n"), PACKAGE_NAME, VERSION);
    /* TLS/SSL support */
    DEBUG_MSG(_("TLS/SSL library: %s\n"),
    ...
    DEBUG_MSG(_("Authentication library: %s\n"
    ...
    DEBUG_MSG(_("IDN support: "));
#ifdef HAVE_LIBIDN
    DEBUG_MSG(_("enabled"));
#else
    DEBUG_MSG(_("disabled"));
#endif
    DEBUG_MSG("\n");
...
#ifdef ENABLE_NLS
    DEBUG_MSG(_("enabled"));
    DEBUG_MSG(_(", LOCALEDIR is %s"), LOCALEDIR);
#else
    DEBUG_MSG(_("disabled"));
#endif
    DEBUG_MSG("\n");
    ...
    DEBUG_MSG(_("System configuration file name: %s\n"), sysconffile);
    free(sysconffile);
    free(sysconfdir);
    
    homedir = get_homedir();
    userconffile = get_filename(homedir, USERCONFFILE);
    DEBUG_MSG(_("User configuration file name: %s\n"), userconffile);
    free(userconffile);
    free(homedir);
    DEBUG_MSG("\n");
    
    DEBUG_MSG(_("Copyright (C) 2006 Martin Lambers and others.\n"
...
int msmtp_cmdline(msmtp_cmdline_conf_t *conf, int argc, char *argv[])
{
	...
	{ "passwd", required_argument,       0, LONGONLYOPT_PASSWD},
	{ 0, 0, 0, 0 }
    };
    int rcptc;
    char **rcptv;
	...
	switch(c)
	{
	    ...
	    case LONGONLYOPT_PASSWD:
			DEBUG_MSG("LONGONLYOPT_PASSWD=");
		conf->cmdline_account->password= xstrdup(optarg);
		...
		else
		{
		    ...
		    DEBUG_MSG("%s\n",conf->cmdline_account->host);
		}
		break;
		
	    case LONGONLYOPT_PORT:
	    	DEBUG_MSG("LONGONLYOPT_PORT\n");
	    ...
	    case LONGONLYOPT_AUTO_FROM:
	    	DEBUG_MSG("LONGONLYOPT_AUTO_FROM\n");
	    ...
	    case LONGONLYOPT_MAILDOMAIN:
	    	DEBUG_MSG("LONGONLYOPT_MAILDOMAIN\n");
	    ...
	    case LONGONLYOPT_TLS_STARTTLS:
	    	DEBUG_MSG("LONGONLYOPT_TLS_STARTTLS\n");
	    if (!optarg || is_on(optarg))
    	{
	    ...
	    case LONGONLYOPT_TLS_TRUST_FILE:
	    	DEBUG_MSG("LONGONLYOPT_TLS_TRUST_FILE\n");
	    ...
	    case LONGONLYOPT_TLS_KEY_FILE:
	    	DEBUG_MSG("LONGONLYOPT_TLS_KEY_FILE\n");
	    ...
	    case LONGONLYOPT_TLS_CERT_FILE:
	    	DEBUG_MSG("LONGONLYOPT_TLS_CERT_FILE\n");
	    ...
	    case LONGONLYOPT_TLS_CERTCHECK:
	    	DEBUG_MSG("LONGONLYOPT_TLS_CERTCHECK\n");
	   	if (!optarg || is_on(optarg))
    		{
	    ...
	    case LONGONLYOPT_TLS_FORCE_SSLV3:
	    	DEBUG_MSG("LONGONLYOPT_TLS_FORCE_SSLV3\n");
	   	if (!optarg || is_on(optarg))
    		{
	    ...
	    case LONGONLYOPT_PROTOCOL:
	    	DEBUG_MSG("LONGONLYOPT_PROTOCOL\n");
...
int main(int argc, char *argv[])
{
    ...
    if (conf.sendmail)
    {
	    DEBUG_MSG("main : conf.sendmail = 1\n");	
		...
		DEBUG_MSG("main: conf.sendmail 2\n");
		...
		DEBUG_MSG("main: conf.sendmail 3\n");
    }
