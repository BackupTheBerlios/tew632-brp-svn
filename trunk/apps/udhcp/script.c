...
/* ------------------------------------------------------- */
	...
	for(;;) {
		switch (type) {
			...
			case OPTION_IP:	/* Works regardless of host byte order. */
				...
				{
					wins_server_ip[index].s_addr=*(uint32_t *)option;
					index++;
					need_nvram_commit=1;
				}
#endif
	...
	if(index && client_config.netbios_enable == 1)
	{
		for(i=0 ; i<index ; i++)
		{
	...
	/* jimmy added 20080718, for checking NetBios info changed */
		if( !netbios_changed ){
			if(index != local_netbios->index){
				netbios_changed = 1;
			}else{
				//while(!netbios_changed && (win_ptr_tmp != NULL)){
				for(i=0 ; i<index && netbios_changed !=1 ; i++){
					already_has = 0;
					win_ptr_tmp = local_netbios->wins_server;
...
//		{
/* ------------------------------------------------------------------------------------ */
	/*jimmy added ,20080721, for wan proto is dhcpc and l2tp, do not "append" physical dns */
		if((strcmp(nvram_safe_get("wan_proto"),"dhcpc") != 0) && (strcmp(nvram_safe_get("wan_proto"),"l2tp") != 0)){
	/* ------------------------------------------------------------- */
