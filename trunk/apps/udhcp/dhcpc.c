...
#define NVRAM_STATIC_ROUTE_NAME "dhcp"
		...
		if((nv_ptr = (char *)nvram_get("static_routing_00"))!=NULL){
			...
			for(s=0; s< CONFIG_STATIC_ROUTE_NUMBER; s++){
...
#define NVRAM_STATIC_ROUTE_NAME "dhcp"
	...
	_system(STATIC_ROUTE_DEL_SHELL);
...
#endif
{
	...
	static const struct option arg_options[] = {
		...
		{"wan_proto",		required_argument,	0, 'w'},
#ifdef UDHCPD_NETBIOS
	...
	while (1) {
		int option_index = 0;
		c = getopt_long(argc, argv, "c:CV:fbH:h:F:i:w:np:qr:s:v:uNRa", arg_options, &option_index);
		if (c == -1) break;
		...
		if (retval == 0) {
			...
			case INIT_SELECTING:
				...
				if (*message == DHCPOFFER) 
				{
					...
					/*NickChou 2008.1.22 
					  for RUSSIA PPTP or PPTP/L2TP in multi-router environment
					*/	
			...
			case REBINDING:
				if (*message == DHCPACK) {
					...
					/* jimmy added, 20080430, to ask httpd reget the data */
					...
					/* ------------------------------------- */
						/* Chun:
						 * a) In russia PPPoE/russia PPTP with WAN_PHY dynamic IP mode, we nned to restart rc 
						...
						while(!send_signal_done)
						{
							...
							if(fp)
							{
								...
								if(!strncmp(data, "i", 1))
								{
									LOG(LOG_DEBUG, "dhcpc sent restart signal to rc");
