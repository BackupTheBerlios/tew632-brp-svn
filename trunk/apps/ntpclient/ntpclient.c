...
int adjust_timezone(void)
{
	...
	DEBUG_MSG("Time update successfully, adjust time. (adjust:%f)\n", time_zone);
        gettimeofday(&tv,&tz);
       	tv.tv_sec = tv.tv_sec+ (time_zone/8)*1800;
       	settimeofday(&tv,&tz);
	// DL 
...
int main(int argc, char *argv[]) {
	...
	for (;;) {
		c = getopt( argc, argv, "c:" DEBUG_OPTION "g:h:i:lp:rst:");
		if (c == EOF) break;
		switch (c) {
			case 'c':
