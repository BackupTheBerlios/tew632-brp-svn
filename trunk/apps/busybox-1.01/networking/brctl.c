...
#ifdef CONFIG_WLAN_GUEST_ZONE
void br_cmd_setguestzone(struct bridge *br, char *ifname, char *arg1)
{
	int err;
	int ifindex;
	
	//printf("NickChou: br_cmd_setguestzone: ifname=%s\n", ifname);
	ifindex = if_nametoindex(ifname);
	if (!ifindex) {
		fprintf(stderr, "setguestzone: interface %s does not exist!\n", ifname);
		return;
	}

	if ((err = br_set_guestzone(br, ifindex)) == 0)
		return;
	else	
		perror("br_set_guestzone");
}	
#endif
...
static struct command commands[] = {
	...
	{1, 1, "stp", br_cmd_stp},
#ifdef CONFIG_WLAN_GUEST_ZONE
	{1, 1, "setguestzone", br_cmd_setguestzone},
#endif	
};
