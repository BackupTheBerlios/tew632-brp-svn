...
//NickChou add for CONFIG_WLAN_GUEST_ZONE
#include "busybox.h"
...
int br_set_guestzone(struct bridge *br, int ifindex)
{
	//printf("NickChou: br_set_guestzone: ifindex=%d\n", ifindex);
	if (br_device_ioctl(br, BRCTL_SET_GUEST_ZONE, ifindex, 0, 0) < 0)
		return errno;
