/* $Id: obsdrdr.c,v 1.1 2007/09/13 09:04:35 ken_chiang Exp $ */
/* MiniUPnP project
...
clear_redirect_rules(void)
{
	...
	if(dev<0)
	{
		if(debug_flag)syslog(LOG_ERR, "open(\"/dev/pf\"): %m");
		return -1;
	}
	...
	{
		if(debug_flag)syslog(LOG_ERR, "ioctl(dev, DIOCXBEGIN, ...): %m");
		goto error;
	}
	if(ioctl(dev, DIOCXCOMMIT, &io) < 0)
	{
		if(debug_flag)syslog(LOG_ERR, "ioctl(dev, DIOCXCOMMIT, ...): %m");
		goto error;
	}
	close(dev);
	return 0;
				   ...
				   const char * desc)
{
	...
	{
		if(debug_flag)syslog(LOG_ERR, "ioctl(dev, DIOCBEGINADDRS, ...): %m");
		r = -1;
	}
	else
	{
		...
		{
			if(debug_flag)syslog(LOG_ERR, "ioctl(dev, DIOCADDADDR, ...): %m");
			r = -1;
		}
		else
		{
			...
			{
            	if(debug_flag)syslog(LOG_ERR, "ioctl(dev, DIOCCHANGERULE, ...) PF_CHANGE_GET_TICKET: %m");
				r = -1;
			}
			else
			{
				...
				if(ioctl(dev, DIOCCHANGERULE, &pcr) < 0)
				{
					if(debug_flag)syslog(LOG_ERR, "ioctl(dev, DIOCCHANGERULE, ...) PF_CHANGE_ADD_TAIL: %m");
					r = -1;
				}
			}
		}
                  ...
                  u_int64_t * packets, u_int64_t * bytes)
{
	...
	{
		if(debug_flag)syslog(LOG_ERR, "ioctl(dev, DIOCGETRULES, ...): %m");
		goto error;
	}
	n = pr.nr;
	for(i=0; i<n; i++)
	{
		...
		{
			if(debug_flag)syslog(LOG_ERR, "ioctl(dev, DIOCGETRULE): %m");
			goto error;
		}
		  ...
		  && (pr.rule.proto == proto) )
		{
			*iport = pr.rule.rpool.proxy_port[0];
			...
			{
				if(debug_flag)syslog(LOG_ERR, "ioctl(dev, DIOCGETADDRS, ...): %m");
				goto error;
			}
			if(pp.nr != 1)
			{
				if(debug_flag)syslog(LOG_NOTICE, "No address associated with pf rule");
				goto error;
			}
			pp.nr = 0;	/* first */
			if(ioctl(dev, DIOCGETADDR, &pp) < 0)
			{
				if(debug_flag)syslog(LOG_ERR, "ioctl(dev, DIOCGETADDR, ...): %m");
				goto error;
			}
...
delete_redirect_rule(const char * ifname, unsigned short eport, int proto)
{
	...
	for(i=0; i<n; i++)
	{
		  ...
		  && (pr.rule.proto == proto) )
		{
			...
			if(ioctl(dev, DIOCCHANGERULE, &pr) < 0)
			{
				if(debug_flag)syslog(LOG_ERR, "ioctl(dev, DIOCCHANGERULE, ...) PF_CHANGE_REMOVE: %m");
				goto error;
			}
			close(dev);
			return 0;
