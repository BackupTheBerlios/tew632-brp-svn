/* $Id: upnphttp.c,v 1.1 2007/09/13 09:04:45 ken_chiang Exp $ */
/* Project :  miniupnp
...
CloseSocket_upnphttp(struct upnphttp * h)
{
	if(close(h->socket) < 0)
	{
		if(debug_flag)syslog(LOG_ERR, "CloseSocket_upnphttp: close(%d): %m", h->socket);
	}
	h->socket = -1;
	h->state = 100;
}
...
Send404(struct upnphttp * h)
{
	...
	if(n < 0)
	{
		if(debug_flag)syslog(LOG_ERR, "Send404: send(http): %m");
	}
...
Send501(struct upnphttp * h)
{
	...
	if(n < 0)
	{
		if(debug_flag)syslog(LOG_ERR, "Send501: send(http): %m");
	}
	CloseSocket_upnphttp(h);
}
...
sendXMLdesc(struct upnphttp * h, char * (f)(int *))
{
	...
	if(!desc)
	{
		if(debug_flag)syslog(LOG_ERR, "Failed to generate XML description");
		return;
	}
...
ProcessHTTPPOST_upnphttp(struct upnphttp * h)
{
	if((h->req_buflen - h->req_contentoff) >= h->req_contentlen)
	{
		if(h->req_soapAction)
		{
			...
			if(debug_flag)syslog(LOG_INFO, "SOAPAction: %.*s",
		...
		else
		{
			...
			if(debug_flag)syslog(LOG_INFO, "No SOAPAction in HTTP headers");
...
ProcessHttpQuery_upnphttp(struct upnphttp * h)
{
	...
	for(i = 0; i<15 && *p != '\r'; i++)
		HttpVer[i] = *(p++);
	HttpVer[i] = '\0';
	if(debug_flag)syslog(LOG_INFO, "HTTP REQUEST : %s %s (%s)",
	...
	else if(strcmp("GET", HttpCommand) == 0)
	{
		...
		{
			if(debug_flag)syslog(LOG_NOTICE, "%s not found, responding ERROR 404", HttpUrl);
			Send404(h);
		}
	}
	else if(strcmp("SUBSCRIBE", HttpCommand) == 0)
	{
		if(debug_flag)syslog(LOG_NOTICE, "SUBSCRIBE not implemented yet");
		Send501(h);
	}
	else
	{
		if(debug_flag)syslog(LOG_NOTICE, "Unsupported HTTP Command %s", HttpCommand);
		Send501(h);
	}
}
...
Process_upnphttp(struct upnphttp * h)
{
	...
	case 0:
		...
		if(n<0)
		{
			if(debug_flag)syslog(LOG_ERR, "recv (state0): %m");
			h->state = 100;
		}
		else if(n==0)
		{
			if(debug_flag)syslog(LOG_WARNING, "HTTP Connection closed inexpectedly");
			h->state = 100;
		}
		else
		{
	...
	case 1:
		...
		if(n<0)
		{
			if(debug_flag)syslog(LOG_ERR, "recv (state1): %m");
			h->state = 100;
		}
		else if(n==0)
		{
		...
		break;
	default:
		if(debug_flag)syslog(LOG_WARNING, "Unexpected state: %d", h->state);
	}
}
...
SendResp_upnphttp(struct upnphttp * h)
{
	...
	if(n<0)
	{
		if(debug_flag)syslog(LOG_ERR, "send(res_buf): %m");
	}
	else if(n < h->res_buflen)
	{
		/* TODO : handle correctly this case */
		if(debug_flag)syslog(LOG_ERR, "send(res_buf): %d bytes sent (out of %d)",
						n, h->res_buflen);
	}
}
