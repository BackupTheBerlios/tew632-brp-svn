      ...
      if (forward)
	{
	  ...
	  /* In strict_order mode, or when using domain specific servers
		...
		if (type != 0  || (daemon->options & OPT_ORDER)){
			...
			DEBUG_P("%s, line %d, strict-order is used\n",__FUNCTION__,__LINE__);
			DEBUG_P("%s, line %d, type = %d, using dns server %s\n",__FUNCTION__,__LINE__,type,inet_ntoa( (&start->addr)->in.sin_addr)    );
  ...
  if (!flags && forward)
	...
	{ 
	  ...
	  if (type == (start->flags & SERV_TYPE) &&
	    ...
	    {
			DEBUG_P("%s, %d, sending query to dns server %s\n",__FUNCTION__,__LINE__,inet_ntoa((&start->addr)->in.sin_addr));
  ...
  if (n >= (int)sizeof(HEADER) && header->qr && 
      (forward = lookup_frec(ntohs(header->id), questions_crc(header, n, daemon->namebuff))))
    {
      struct server *server = forward->sentto;
		/*
			Note:
				/opt/ap71_tools/mips32k/include/arpa/nameser.h
				SERVFAIL	: 2
				NOTIMP		: 4
				REFUSED		: 5
		*/
/* jimmy modified 20080513, for cdrouter */
//      if ((header->rcode == SERVFAIL || header->rcode == REFUSED) &&
//	  !(daemon->options & OPT_ORDER) &&
//	  forward->forwardall == 0)

/* jimmy modified 20080611, for russia */
//      if ((header->rcode == SERVFAIL || header->rcode == REFUSED || header->rcode == NOTIMP) &&
//	  !(daemon->options & OPT_ORDER) &&
//	  forward->forwardall == 0)

	if(  	( (daemon->options & OPT_ORDER) && (header->rcode != NOERROR) &&			// Russia mode
				(forward->sentto->next) && (forward->forwardall == 0) ) 				// use strict_order
			||
			( !(daemon->options & OPT_ORDER) && // Non Russia mode, to pass cdrouter dns fail over
				(header->rcode == SERVFAIL || header->rcode == REFUSED || header->rcode == NOTIMP) && 
				 (forward->forwardall == 0) )						
		)
/* ------------------------------------- */
...
#ifdef DEBUG_DNSMASQ
{
	...
	tmp = daemon->servers;
	DEBUG("%s, we have dns server :\n",__FUNCTION__);
	while(tmp){
		DEBUG("%s\n",__FUNCTION__,inet_ntoa( (&tmp->addr)->in.sin_addr)    );
		tmp = tmp->next;
	}
}
#endif
  ...
  if (extract_request(header, (size_t)n, daemon->namebuff, &type))
    {
		DEBUG_P("%s, line %d, receiving query for %s\n",__FUNCTION__,__LINE__,daemon->namebuff);
      if (listen->family == AF_INET) 
