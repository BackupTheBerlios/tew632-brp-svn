...
<title>TRENDNET | TEW-632BRP | Status | Device Information</title>
...
<script language="JavaScript">
 ...
 var wTimesec, wan_time;
	...
	function set_iface_info()
	{
		...
		position = document.getElementById( 'wan_ip' );
		position.innerHTML = dat[0];
		position = document.getElementById( 'wan_netmask' );
		position.innerHTML = dat[1];
		position = document.getElementById( 'wan_gateway' );
		position.innerHTML = dat[2];			
		position = document.getElementById( 'wan_dns' );	  	                  
						  	
		if(dat[3] !="" && dat[3] !="0.0.0.0" && dat[4] !="" && dat[4] !="0.0.0.0"){
			position.innerHTML = dat[3] + "," + dat[4];
		}else if(dat[3] !="" && dat[3] !="0.0.0.0"){
			position.innerHTML = dat[3];
		}else if(dat[4] !="" && dat[4] !="0.0.0.0"){
			position.innerHTML = dat[4];
		}else{
			position.innerHTML = "0.0.0.0";
		}						
	}				  
	...
	if (init_status == "")
	{
		...
		document.getElementById("wan_netmask").innerHTML = "0.0.0.0";
		document.getElementById("wan_gateway").innerHTML = "0.0.0.0";
		document.getElementById("wan_dns1").innerHTML = "0.0.0.0";
	...
	if (xmlhttp.readyState == 4 && xmlhttp.status == 200){ 
		...
		if(get_by_id("wan_proto").value == "static"){
			...
			position.innerHTML = "DHCP Client " + con_status + " <input type=\"button\" value=\"DHCP Release\" name=\"release\" onClick=\"dhcp_release()\">&nbsp;<input type=\"button\" value=\"DHCP Renew\" name=\"renew\" id=\"renew\" onClick=\"dhcp_renew()\">";		
		}else if(get_by_id("wan_proto").value == "pppoe"){			
			if(get_by_id("wan_pppoe_dynamic_00").value == "0")
				fix_or_dynamic_ip = "Fixed IP ";
			...
			position.innerHTML = "Bigpond Client " + con_status + " <input type=\"button\" value=\"Bigpond Login\" name=\"bpalogin\" onClick=\"bigpond_connect()\">&nbsp;<input type=\"button\" value=\"Bigpond Logout\" name=\"bpalogout\" id=\"bpalogout\" onClick=\"bigpond_disconnect()\">";			
		}else{
			alert("connect type none");
		}
		//show phyical IP (PPTP/PPPoE)
		if((get_by_id("wan_proto").value == "pptp"||get_by_id("wan_proto").value == "pppoe")&&(xmlhttp.responseXML.getElementsByTagName("wan_ip")[0].firstChild.nodeValue == "0.0.0.0"))
			{	
			...
			if(dns1 !="(null)" && dns1 !="0.0.0.0" && dns2 !="(null)" && dns2 !="0.0.0.0"){
				dns1.innerHTML = dns1_value ;
				dns2.innerHTML = ","+dns2_value ;
			}else if(dns1 !="(null)" && dns1 !="0.0.0.0" ){
				dns1.innerHTML = dns1;
			}else if(dns2 !="(null)" && dns2 !="0.0.0.0" ){	
				dns2.innerHTML = dns2_value;
		}else{
                    ...
                    <table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#FFFFFF">
                        <tr>
                          <td colspan="2" bgcolor="#FFFFFF"><span class="bluetextbold3">Firmware Version:
                            <% CmoGetStatus("version"); %>.<% CmoGetStatus("build"); %>,  <% CmoGetStatus("version_date"); %></span></td>
                        </tr>
                      ...
                      <table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#FFFFFF">
                        ...
                        <tr>
                          ...
                          <td width="78%" class="bggrey">
                          <script>
								var temp_ssid="<% CmoGetCfg("wlan0_ssid","none"); %>";
								var replace_str1=temp_ssid.replace(/</gm,"&lt");
								var replace_str2=replace_str1.replace(/>/gm,"&gt");
								var replace_str3=replace_str2.replace(/\"/gm,"&quot");
								document.write(replace_str3);										
						  </script></td>
                        </tr>
                        <tr>
